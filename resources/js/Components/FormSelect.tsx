    ...
    return (
        <div className={`relative ${className}`} ref={containerRef}>
            ...
            <Transition
                ...
                leaveTo="opacity-0"
            >
                <div className="absolute z-50 mt-1 w-full min-w-max bg-white dark:bg-gray-800 shadow-xl max-h-60 rounded-xl py-1 text-base ring-1 ring-black/5 dark:ring-white/10 overflow-auto focus:outline-none sm:text-sm custom-scrollbar">
                    {options.map((option) => (
                        <div
                            key={option.value}
                            onClick={() => {
                        ...
                        >
                            <span className={`block whitespace-nowrap ${value === option.value ? 'font-medium' : 'font-normal'}`}>
                                {option.label}
                            </span>
                            {value === option.value && (
